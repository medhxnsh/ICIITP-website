...
export default async function PortfolioSchemePage({ params }: Props) {
  ...
  const startups = await getPublishedStartups("sisf").catch(() => []);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-[96px] pb-12">
      <Breadcrumb items={[{ label: "Home", href: "/" }, { label: "Portfolio", href: "/portfolio" }, { label: "SISF" }]} />
      <header className="mb-10">
        <h1 className="text-2xl sm:text-4xl font-black text-[var(--color-brand-800)] mb-4">SISF Portfolio</h1>
