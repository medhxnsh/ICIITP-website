...
import { ArrowRight, Banknote, Clock, Rocket } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Acceleration Programs — IC IITP",
  description: "Structured acceleration at IC IITP — business and technical acceleration for revenue-stage startups ready to scale.",
};
...
export default async function AccelerationPage({ params }: Props) {
  ...
  const category = PROGRAM_CATEGORIES[2];
  const programs = await getProgramsBySection("ACCELERATION").catch(() => []);
    ...
    <>
      <div className="relative overflow-hidden" style={{ background: "linear-gradient(160deg, var(--color-hero-from) 0%, var(--color-hero-via) 60%, var(--color-hero-to) 100%)" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20 relative z-10">
          <Breadcrumb items={[{ label: "Home", href: "/" }, { label: "Programs", href: "/programs" }, { label: "Acceleration" }]} variant="light" />
          <p className="text-xs font-semibold uppercase tracking-widest mb-4 mt-6 text-orange-200">
            <Rocket className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" aria-hidden="true" />
            Stage 3 · {category.subtitle}
          </p>
          ...
          {programs.map((program) => (
            <StaggerItem key={program.slug}>
              <Link
                ...
                className="group relative flex rounded-2xl bg-white overflow-hidden transition-all duration-500 ease-out hover:-translate-y-1.5 shadow-[0_2px_8px_rgba(28,46,6,0.06),0_8px_24px_rgba(28,46,6,0.05)]"
              ...
              >
                ...
                <div className="relative flex-1 flex items-start gap-6 p-6 sm:p-8">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-1 mt-3">
                      {getProgramBadge(program.slug, program.badge) && (
                        <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ backgroundColor: accent + "18", color: accent }}>
                          {getProgramBadge(program.slug, program.badge)}
                        </span>
                      )}
                    </div>
                    <h2 className="text-lg sm:text-xl font-black leading-snug mt-2 mb-3" style={{ color: "var(--color-brand-950)" }}>{program.title}</h2>
